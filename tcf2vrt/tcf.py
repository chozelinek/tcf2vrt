...
#         print etree.tostring(self.xmltree, encoding='unicode', method='xml',pretty_print=True)
    ...
    def get_tokens(self):
        ...
        for i in tokens:
            tokenID = i.attrib["ID"]
            word = i.text
            self.tok_dic[tokenID] = OrderedDict([('word',word)])
        pass
    ...
    def get_dependencies(self):
        if self.layer_is_present('depparsing'):
            depparsing_layer = self.tree.find('.//{}{}'.format(self.tc,'depparsing'))
            self.tagsets.append(('depparsing',depparsing_layer.attrib['tagset']))
            dependencies = self.tree.findall('.//{}{}'.format(self.tc,'dependency'))
            for i in dependencies:
                tokenID = i.attrib['depIDs']
                self.tok_dic[tokenID]['func'] = i.attrib['func']
                if "govIDs" in i.attrib.keys():
                    self.tok_dic[tokenID]['gov'] = i.attrib['govIDs']
                else:
                    self.tok_dic[tokenID]['gov'] = ''
                self.tok_dic[tokenID]['dep'] = i.attrib['depIDs']
        else:
            pass
...
#         self.cmd = "{http://www.clarin.eu/cmd/}"
#         self.md = "{http://www.dspin.de/data/metadata}"
#         self.positional_elements = ["POStags","lemmas","depparsing"]
#         self.structural_elements = ["textstructure","sentences","namedEntities"]
#         self.tok_dic = {}
#         self.tagsets = []
#         self.language = ''
...
#             depparsing_layer = self.tree.find('//{}{}'.format(self.tc,'depparsing'))
#             self.tagsets.append(('depparsing',depparsing_layer.attrib['tagset']))
#             for i in depparsing_layer:
#                 tokenID = i.attrib['govIDs']
#                 self.tok_dic[tokenID]['func'] = i.attrib['func']
#                 self.tok_dic[tokenID]['gov'] = i.attrib['govIDs']
#                 self.tok_dic[tokenID]['dep'] = i.attrib['depIDs']
#         else:
